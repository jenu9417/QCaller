...
// BulkCreateContact : handles the request, validates, operates and sends response
func BulkCreateContact(r *http.Request, ctx types.Context) contract.Response {
	request := &contract.BulkCreateContactRequest{}
	response := &contract.BulkCreateContactResponse{}
	...
	if err := core.BulkCreateContact(ctx, request, response); err != nil {
		return util.FailureResponse(ctx, response, err.HTTPCode, err)
	}
