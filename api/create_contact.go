...
// CreateContact : handles the request, validates, operates and sends response
func CreateContact(r *http.Request, ctx types.Context) contract.Response {
	contact := &model.Contact{}
	response := &contract.CreateContactResponse{}
	...
	request := &contract.CreateContactRequest{Contact: contact}
	...
	if err := core.CreateContact(ctx, request, response); err != nil {
		return util.FailureResponse(ctx, response, err.HTTPCode, err)
	}
